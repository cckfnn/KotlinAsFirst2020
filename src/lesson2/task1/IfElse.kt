...
fun timeForHalfWay(
    ...
    return when {
        ...
        halfDistance <= s1 + s2 -> (halfDistance - s1) / v2 + t1
        halfDistance > s1 + s2 -> (halfDistance - s1 - s2) / v3 + t1 + t2
        else -> Double.NaN
    }
}
...
fun segmentLength(a: Int, b: Int, c: Int, d: Int): Int {
    ...
    if ((result == 0) and ((b !in c..d) and (d !in a..b))) result = -1
    return result
}
